    ...
    <main className="min-h-screen">
      ...
      <section style={{ padding: '0 clamp(20px,5vw,56px) var(--pad-v)' }}>
        <div style={{ maxWidth: '1140px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div className="glass-panel" style={{ overflow: 'hidden' }}>
            <div style={{ padding: '16px 24px', borderBottom: '1px solid rgba(42,31,20,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h2 style={{ fontFamily: 'var(--font-serif)', fontWeight: 700, color: 'var(--brown-dark)', fontSize: '1.1rem' }}>
                All Events
                {events.length > 0 && <span style={{ marginLeft: '8px', fontSize: '0.85rem', fontFamily: 'var(--font-sans)', fontWeight: 400, color: 'var(--text-mid)' }}>({events.length})</span>}
              </h2>
            </div>
            ...
            {eventsLoading ? (
              ...
              <div>
                {events.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE).map((ev) => (
                ...
                {events.length > PAGE_SIZE && (
                  <div style={{ padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid rgba(42,31,20,0.08)' }}>
                    ...
                    <span style={{ fontSize: '0.8rem', color: 'var(--text-mid)', fontFamily: 'var(--font-sans)' }}>
                      Page {currentPage} of {Math.ceil(events.length / PAGE_SIZE)}
                    </span>
                    <button
                      onClick={() => setCurrentPage(p => Math.min(Math.ceil(events.length / PAGE_SIZE), p + 1))}
                      disabled={currentPage === Math.ceil(events.length / PAGE_SIZE)}
                      style={{ padding: '6px 16px', borderRadius: '8px', fontSize: '0.8rem', fontWeight: 500, border: '1px solid rgba(42,31,20,0.15)', color: currentPage === Math.ceil(events.length / PAGE_SIZE) ? 'var(--text-mid)' : 'var(--brown-dark)', background: 'rgba(255,248,238,0.5)', cursor: currentPage === Math.ceil(events.length / PAGE_SIZE) ? 'not-allowed' : 'pointer', opacity: currentPage === Math.ceil(events.length / PAGE_SIZE) ? 0.4 : 1, fontFamily: 'var(--font-sans)', transition: 'background 0.15s ease' }}
                      onMouseEnter={e => { if (currentPage !== Math.ceil(events.length / PAGE_SIZE)) (e.currentTarget as HTMLElement).style.background = 'rgba(255,248,238,0.9)' }}
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(255,248,238,0.5)' }}
                    >Next →</button>
                  </div>
                )}
      ...
      {toast && (
        <div
          ...
          className={`fixed bottom-6 right-6 z-50 rounded-xl text-white text-sm font-medium shadow-lg ${toast.type === 'success' ? 'bg-[var(--orange)]' : 'bg-red-500'}`}
        >
          {toast.msg}
        </div>
      )}
